...
function toBackendInwardSaved(
  ...
  return {
    ...
    items: entry.items.map((item) => ({
      ...
      godownQtys: [
        { godownId: "main", qty: BigInt(Math.round(item.godownQty)) },
      ],
    })),
  };
}
...
export default function App() {
  ...
  const setTransactionsWithBackend = async (updaterOrArray: any) => {
    setTransactions(updaterOrArray);
  };
  ...
  const setUsersWithBackend: React.Dispatch<React.SetStateAction<AppUser[]>> = (
    ...
    for (const u of deleted) {
      ...
      actor.deleteUser(backendId).catch(console.error);
    }
  ...
  > = (updater) => {
    ...
    for (const b of added) actor.addBusiness(b.id, b.name).catch(console.error);
    for (const b of deleted) actor.deleteBusiness(b.id).catch(console.error);
    for (const b of updated)
      actor.updateBusiness(b.id, b.name).catch(console.error);
  };
  ...
  > = (updater) => {
    ...
    for (const name of added) {
      ...
      actor.addGodown(id, name, activeBusinessId).catch(console.error);
    }
    for (const name of deleted) {
      const mapping = godownMapRef.current[name];
      if (mapping) actor.deleteGodown(mapping.id).catch(console.error);
    }
  };
  ...
  > = (updater) => {
    ...
    for (const c of added) {
      ...
      actor.addCategory(id, c.name).catch(console.error);
    ...
    for (const c of updated) {
      ...
      for (const s of oldSubs)
        actor.deleteSubCategory(catId, s.id).catch(console.error);
      for (const s of newSubs)
        actor.addSubCategory(catId, s).catch(console.error);
    }
  };
  ...
  > = (updater) => {
    ...
    for (const p of added) {
      ...
      actor.addBiltyPrefix(id, p).catch(console.error);
    }
    for (const p of deleted) {
      const id = biltyPrefixIdMapRef.current[p] || p;
      actor.deleteBiltyPrefix(id).catch(console.error);
    }
  };
  ...
  > = (updater) => {
    ...
    for (const k of added) {
      ...
      actor.addTransportTracker(id, k, next[k]).catch(console.error);
    }
    for (const k of deleted) {
      const id = transportTrackerIdMapRef.current[k] || k;
      actor.deleteTransportTracker(id).catch(console.error);
    }
    for (const k of updated) {
      const id = transportTrackerIdMapRef.current[k] || k;
      actor.updateTransportTracker(id, k, next[k]).catch(console.error);
    }
  };
  ...
  > = (updater) => {
    ...
    for (const t of added)
      (actor as any).addTransitEntry(toBackendTransit(t)).catch(console.error);
    for (const t of deleted)
      (actor as any).deleteTransitEntry(String(t.id)).catch(console.error);
  ...
  > = (updater) => {
    ...
    for (const p of added)
      (actor as any).addQueueEntry(toBackendQueue(p)).catch(console.error);
    for (const p of deleted)
      (actor as any).deleteQueueEntry(String(p.id)).catch(console.error);
    for (const p of updated)
      (actor as any).updateQueueEntry(toBackendQueue(p)).catch(console.error);
  };
  ...
  > = (updater) => {
    ...
    for (const e of added)
      (actor as any).saveInward(toBackendInwardSaved(e)).catch(console.error);
    for (const e of deleted)
      (actor as any).deleteInwardSaved(String(e.id)).catch(console.error);
  ...
  > = (updater) => {
    ...
    for (const k of added)
      (actor as any)
        ...
        .catch(console.error);
    for (const k of deleted)
      (actor as any).deleteInventoryItem(k).catch(console.error);
    for (const k of updated)
      (actor as any)
